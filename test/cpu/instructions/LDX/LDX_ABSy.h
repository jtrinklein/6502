...
class LDX_Absolute_Y_Tests : public CxxTest::TestSuite 
{
public:
    CPU*  cpu;
    Mem* mem;
    static constexpr Byte opcode = 0xBE;
    ...
    void test_WithZero( void ) {
        ...
        TS_ASSERT_EQUALS(cpu->X, zero_val);
    ...
    void test_WithNegative( void ) {
        ...
        TS_ASSERT_EQUALS(cpu->X, neg_val);
        TS_ASSERT_EQUALS(cycles, op_cycles);
        TS_ASSERT_EQUALS(cpu->PC - addr, op_size);
